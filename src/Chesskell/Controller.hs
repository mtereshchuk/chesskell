...
updateAppState (UI.EventKey (UI.SpecialKey UI.KeyLeft) UI.Down (UI.Modifiers UI.Up UI.Up UI.Up) _) appState =
  if appState^.moveNum == 0
  then appState
  else moveNum %~ (\num -> num - 1) $ appState
updateAppState (UI.EventKey (UI.SpecialKey UI.KeyRight) UI.Down (UI.Modifiers UI.Up UI.Up UI.Up) _) appState =
...
updateAppState (UI.EventKey (UI.SpecialKey UI.KeyLeft) UI.Down (UI.Modifiers UI.Down UI.Up UI.Up) _) appState =
  if appState^.gameNum == 0
  then appState
  else moveNum .~ 0 $ (gameNum %~ (\num -> num - 1) $ appState)
updateAppState (UI.EventKey (UI.SpecialKey UI.KeyRight) UI.Down (UI.Modifiers UI.Down UI.Up UI.Up) _) appState =
